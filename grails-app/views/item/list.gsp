			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="state" title="${message(code: 'item.state.label', default: 'State')}" />
				...
				<g:each in="${itemInstanceList}" status="i" var="itemInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${itemInstance.id}">${fieldValue(bean: itemInstance, field: "sku")}</g:link></td>
					
						<td>${fieldValue(bean: itemInstance, field: "description")}</td>
					
						<td>${fieldValue(bean: itemInstance, field: "status")}</td>
					
						<td>${fieldValue(bean: itemInstance, field: "state")}</td>
